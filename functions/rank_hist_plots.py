...
def rank_maps(single_phase,vmax = 0.6, step = 0.1, savefig = 0, savetitle = '', savedir  = ''):
    ...
    fig.suptitle(savetitle, fontsize = 25)
    
#     vmax = 
    
    for bin_ in single_phase.bin_.values:
        sb = single_phase.sel(bin_ = bin_)
    ...
    #     sb = sb.where(sb < vmax, 0)
        ...
        ax = fig.add_subplot(gs[bin_ + 3], projection = ccrs.PlateCarree())
        pdata = sb.hist_.plot(ax = ax, add_colorbar = False, vmax = vmax, cmap = cmap_custom)

        ax.set_title('')
        ax.coastlines(resolution = '50m')
        ax.annotate(rh.ordinal(bin_), xy = (0.5, -0.1), xycoords = 'axes fraction', ha = 'center', size = 15)
    ...
    if savefig:
        fig.savefig(savedir + savetitle + '.png')
...
def generate_phase_hist(data_total,  savedir = '', save_name = ''):
    ...
    for plot_num,phase in enumerate(phases):
        ...
        vals = data.hist_.values
        plt.bar(data.bin_, vals, color = best_blue)


        ax.set_title(phase.capitalize())
        ax.set_xticks(data.bin_)
        ax.set_xticklabels((np.array(data.bin_) + 1).astype(str))
