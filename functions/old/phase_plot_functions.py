...
def trend_plots(data,
                        titlepiece = 'Mystery', datasource = 'AWAP', colorbar_title = 'coolio', 
    ...
    cmap_custom_RdBu = mpc.LinearSegmentedColormap.from_list("RdWtBu", custom_cmap,len(levels))
    ...
    for i,phase in enumerate(phases):
        ...
        ax = fig.add_subplot(gs[i + 3], projection  = ccrs.PlateCarree())
        plot = data_phase.plot(ax = ax,cmap = cmap_custom_RdBu, add_colorbar = False, vmax = vmax, vmin = vmin)
